...
注：JVM 在执行类的初始化期间，会先去获取一个初始化锁（Class 对象的初始化锁，每一个类或接口都只有一个与之对应的初始化锁），这个锁可以同步多个线程对同一个类的初始化（第一次要初始化时，会先标记为 state = initializing 并释放初始化锁，而其他线程获取锁后发现初始化中会再释放并进行等待；初始化后，即执行静态初始化和初始化类中声明的静态字段后，标记为已初始化，并唤醒等待的所有线程；其他线程获取锁而发现初始化已完成，就会直接释放初始化锁。第一次初始化共经理两次锁的获取和释放，state = initialized 之后其他线程则只需要经理一次锁的获取和释放。），即是线程安全的。
