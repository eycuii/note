...
阻塞 I/O，不管是磁盘 I/O 还是网络 I/O，数据在写入 OutputStream 或者从 InputStream 读取时都有可能阻塞。一旦阻塞，线程将失去 CPU 的使用权。

当前网络 I/O 也有解决方法，如一个客户端对应一个处理线程，阻塞时不会影响其他线程，而且使用线程池来减少线程创建、回收的成本。但是有可能需要大量 HTTP 长连接的情况，或者每个线程需要访问一些共享资源时的同步问题等，还是需要另一种新的 I/O 方式。

### NIO 的工作机制

![深入分析JavaWeb技术内幕-NIO相关类图](../img/深入分析JavaWeb技术内幕-NIO相关类图.png)

关键类：Channel、Selector。

Channel 比 Socket 更加具体，可以把它比作某种具体的交通工具，如汽车、高铁。

而 Selector 可比作一个车站的车辆运行调度系统，负责监控每辆车的运行状态。即可以轮询每个 Channel 状态。

Buffer 类：比 Stream 更加具体，可以比作车上的座位，如果 Channel 是一个高铁，Buffer 就是高铁上的座位。而 Stream 只代表一个座位，并不知道是汽车还是高铁，也不知道是否有座位，而这些信息都被封装在 Socket 运输工具里面。

NIO 引入了 Channel、Buffer、Selector 就是想把这些信息具体化，让程序员有机会控制它们。例如，当调用 write() 往 SendQ 中写数据时，当一次写的数据超过 SendQ 长度时需要按照 SendQ 长度进行分割，在这个过程中需要将用户空间数据和内核地址空间进行切换，而这个切换不是你可以控制的，但在 Buffer 中我们可以控制 Buffer 的容量、是否扩容以及如何扩容。

具体代码实现（一个 Selector 监听处理多个 Channel 部分）：

1. 用 Selector 的静态工厂创建一个选择器。
2. 创建一个服务端的 Channel（ServerSocketChannel），设置为非阻塞模式，绑定一个 Socket 对象，并把这个通信信道注册到选择器上。
3. select() 选择一组准备就绪的键，然后调用选择器 selector 的 selectedKeys() 方法来获取并循环检查注册到这组键上的所有 Channel（通过 selectionKey 可获取它的 Channel）是否有需要的事件发生。如果状态是可接收的，就调用这个 channel 的 accept() 方法来接收一个 SocketChannel，并把该 SocketChannel 注册到 selector 里。如果是可读的，就调用 read() 方法读取 socketChannel 里的数据。

在上面这个流程中，有两个线程：一个专门负责监听客户端连接请求的线程（阻塞执行）；另一个是专门负责处理请求的线程，这个线程才会真正采用 NIO 的方式，像 Tomcat、Jetty 都是使用这个处理方式。

-----------------------

https://mp.weixin.qq.com/s/BZJiUMR60YwpW1krN3Me_g ：

对于 select()，会阻塞，但不会一直阻塞，在一定时间内没有（超时）就会返回。JDK 1.5 后用 epoll（让内核监听并回调）替换了 select/poll（内核反复遍历文件描述符）。

channel.read(buffer)：通过 buffer 读取 socket 的数据。（同步）

Java NIO 的非阻塞其实体现在 read、write（轮询调用 select() ，每次根据返回信息进行读、写。读写完就返回）。

而说的同步就是主要指 select() 方法里使用的 I/O 多路复用，在 I/O 操作阶段（即内核到用户空间的数据复制过程，recvfrom 这个 system call）阻塞了（这种就叫同步 https://segmentfault.com/a/1190000003063859）。
...
- mark：当前 position 的前一个位置或者默认是 0。reset() 方法中可以通过 mark 将position 恢复到 mark 位置。

 可通过 ByteBuffer.allocate(n) 方法创建 n 个 byte 的缓冲区数组。

byteBuffer.flip()：limit 变成 position，position 变零。从写模式变为读模式。

注：通过 Channel 获取的 I/O 数据，首先会经过操作系统的 Socket 缓冲区，再将数据复制到 Buffer 里。这个操作系统缓冲区就是 TCP 所关联的 RecvQ 或者 SendQ 队列。

从系统缓冲区到用户缓冲区的复制过程比较耗性能，Buffer 提供了另一种直接操作操作系统缓冲区的方式，即 ByteBuffer.allocateBirector(n)。它返回的就是底层存储空间相关的缓冲区，它通过 native 代码操作而并非 JVM 堆里的内存空间。每次创建、释放都调用一次 System.gc()。注：使用 DirectByteBuffer 会引起 JVM 内存泄漏问题。
...
与传统的文件访问方式相比，可减少从内核到用户空间的数据复制，可让数据之间在内核空间中移动。

FileChannel.map

将文件按一定大小块映射到内存区域。当程序访问这个内存区域时直接操作对应的文件数据。减少复制。
...
Linux：iostat 命令（查看 I/O wait 指标）。
...
InputStream 到 Reader 时用的 InputStreamReader 适配器

​    

## 设计模式解析之装饰器模式

FilterInputStream 是 InputStream 的装饰器接口。
